...
ialazor.digiline_handler_get = function(pos, node, channel)
	...
	local resp = {
		...
		dir = ialazor.facedir_to_down_dir(node.param2),
		name = node.name,
		origin = channel,
		pos = pos
	}

	digilines.receptor_send(pos, ialazor.digiline_rules, channel, resp)
end

ialazor.digiline_handler_fire = function(pos, node, channel, msg)
	local meta = minetest.get_meta(pos)
	...
	-- TODO: Modify "ialazor.fire" to return success/failure, so we can
	...
	-- We cannot directly call "ialazor.fire", as we don't know the
...
ialazor.digiline_effector = function(pos, node, channel, msg)
	...
	if msg.command == "get" then
		ialazor.digiline_handler_get(pos, node, channel, msg)
	elseif msg.command == "fire" then
		ialazor.digiline_handler_fire(pos, node, channel, msg)
	end
end
