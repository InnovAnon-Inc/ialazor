...
local register_spacecannon_3 = function(def)

	local entity_texture = "energycube_" .. def.color .. ".png"

	minetest.register_entity("ialazor:fireenergycube_" .. def.color, {
		initial_properties = {
			visual = "cube",
			visual_size = {x=0.25, y=0.25},
			textures = {
		...
		on_step = function(self, dtime)
			...
			if goes_through then
				...
				if collided then
					ialazor.destroy_4(pos, def.range, def.intensity)
					self.penetrated = self.penetrated + 1
					if self.penetrated >= def.penetration then
						self.object:remove()
					end
	...
	local def_cannon = {
		...
		tiles = textures,
		groups = def.is_th and groups_base or groups_rail,
		drop = "ialazor:firecannon_" .. def.color,
		sounds = default.node_sound_glass_defaults(),
		paramtype2 = "facedir",
		legacy_facedir_simple = true,
		...
		mesecons = {effector = {
			action_on = function (pos)
				local meta = minetest.get_meta(pos)
				local owner = meta:get_string("owner")
				ialazor.fire_3(pos, owner, def.color, def.speed, def.is_th, def.storage_require_mod)
			end
		}},
		...
		on_receive_fields = function(pos, _, fields, sender)
			...
			if fields.fire then
				ialazor.fire_3(pos, playername, def.color, def.speed, def.is_th, def.storage_require_mod)
			end
	...
	minetest.register_node("ialazor:firecannon_" .. def.color, def_cannon)

	technic.register_machine("HV", "ialazor:firecannon_" .. def.color, technic.receiver)

	minetest.register_craft({
		output = 'ialazor:firecannon_' .. def.color,
		recipe = {
			{'', 'default:steelblock', ''},
			{ def.ingredient, def.ingredient, def.ingredient},
			{'', 'default:steelblock', ''}
...
local register_spacecannon_4 = function(def)

	local entity_texture = "energycube_" .. def.color .. ".png"

	minetest.register_entity("ialazor:fireenergycube2_" .. def.color, {
		initial_properties = {
			visual = "cube",
			visual_size = {x=0.25, y=0.25},
			textures = {
		...
		on_step = function(self, dtime)
			...
			if goes_through then
				...
				if collided then
					ialazor.destroy_5(pos, def.range, def.intensity)
					self.penetrated = self.penetrated + 1
					if self.penetrated >= def.penetration then
						self.object:remove()
					end
	...
	local def_cannon = {
		...
		tiles = textures,
		groups = def.is_th and groups_base or groups_rail,
		drop = "ialazor:firecannon2_" .. def.color,
		sounds = default.node_sound_glass_defaults(),
		paramtype2 = "facedir",
		legacy_facedir_simple = true,
		...
		mesecons = {effector = {
			action_on = function (pos)
				local meta = minetest.get_meta(pos)
				local owner = meta:get_string("owner")
				ialazor.fire_4(pos, owner, def.color, def.speed, def.is_th, def.storage_require_mod)
			end
		}},
		...
		on_receive_fields = function(pos, _, fields, sender)
			...
			if fields.fire then
				ialazor.fire_4(pos, playername, def.color, def.speed, def.is_th, def.storage_require_mod)
			end
	...
	minetest.register_node("ialazor:firecannon2_" .. def.color, def_cannon)

	technic.register_machine("HV", "ialazor:firecannon2_" .. def.color, technic.receiver)

	minetest.register_craft({
		output = 'ialazor:firecannon2_' .. def.color,
		recipe = {
			{'', 'default:steelblock', ''},
			{ def.ingredient, def.ingredient, def.ingredient},
			{'', 'default:steelblock', ''}
...
cannon.register_spacecannon_3 = register_spacecannon_3
cannon.register_spacecannon_4 = register_spacecannon_4
return cannon
